...
class PreSet(utils.HashableElement):
    ...
    def __repr__(self):
        return f"PreSet(name=\"{self._name}\", description=\"{self._description}\", metrics={self._metric_tags}, train_ratio={self._train_ratio}, balance_ratio={self._balance_ratio}, use_boolean_model={self._use_boolean_model}, pca_features={self._pca_features})"
...
TRAIN_50 = PreSet("train_05", "Training size 50%", train_ratio=0.5)
TRAIN_60 = PreSet("train_06", "Training size 60%", train_ratio=0.6)
TRAIN_80 = PreSet("train_08", "Training size 80%", train_ratio=0.8)
...
TRAIN_50_BOOLEAN = PreSet("train_05_boolean", "Training size 50% with boolean model", train_ratio=0.5, use_boolean_model=True)
TRAIN_60_BOOLEAN = PreSet("train_06_boolean", "Training size 60% with boolean model", train_ratio=0.6, use_boolean_model=True)
TRAIN_80_BOOLEAN = PreSet("train_08_boolean", "Training size 80% with boolean model", train_ratio=0.8, use_boolean_model=True)
